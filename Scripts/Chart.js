...
var Chart = function(context){
	...
	var Radar = function (data,config,ctx) {
		...
		if (!config.scaleOverride){
			...
			for (var i=1; i<calculatedScale.steps+1; i++){
				if(labelTemplateString){
				calculatedScale.labels.push(tmpl(labelTemplateString,{value:(config.scaleStartValue + (config.scaleStepWidth * i)).toFixed(getDecimalPlaces (config.scaleStepWidth))})); //TODO!!!
				}
			}
		}
